...
def ninjaGenVersionInfo(cur, target):
    ...
    '''
    res = cur.build(target, 'GEN_VERSION_INFO')
    return res
...
def shogunTFFrame(argv):
    ...
    cursor = Writer(open(ag.o, 'w'))
    cursor.comment(f'automatically generated by {__file__}')
    cursor.variable('CXXFLAGS', '-w -std=c++14 -O2 -fPIC -I. -lpthread -lprotobuf'
            + ' -I./debian/embedded/eigen/ -I./third_party/eigen3/ ')
    ...
    cursor.rule('GEN_VERSION_INFO', f'sh {ag.B}/tensorflow/tools/git/gen_git_source.sh $out')
    srclist.extend(ninjaGenVersionInfo(cursor, genlist[0]))
    ...
    # link the final executable
    cursor.rule('CXX_SHLIB', f'g++ -shared -fPIC $CXXFLAGS $in -o $out')
